...
describe("SourceCode", () => {
    ...
    describe("getJSDocComment()", () => {
        ...
        it("should not get JSDoc comment for class property even if the class has jsdoc present", () => {
            ...
            linter.defineRule("checker", () => ({ ArrowFunctionExpression: spy }));
            linter.verify(code, { rules: { checker: "error" }, parser: "babel-eslint", parserOptions: { ecmaVersion: 6, ecmaFeatures: { jsx: true } } });
            assert.isTrue(spy.calledOnce, "Event handler should be called.");
        });
